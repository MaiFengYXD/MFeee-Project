...
local function MiniNotify(Title, Text)
...
if RunService:IsStudio() then
    return MiniNotify("What The Fuck Are You Doing?", "Stop Cheating In Studio 😂")
elseif not (makefolder and isfile and readfile and writefile) then
    return MiniNotify("Incompatible Executor!", "(Missing makefolder, isfile, readfile, writefile)")
elseif not getgenv then
    return MiniNotify("Incompatible Executor!", "(Missing getgenv)")
elseif game.PlaceId ~= 3297964905 then
    return MiniNotify("Incompatible Game!", "This Script Only Works On Weaponry")
end
...
local CurrentVersion = "Early Alpha 1"
local Library, ThemeManager, SaveManager, Universals, Aimbot, SilentAim, ESPLibrary, LatestVersion, SoundService, PrintUI
...
if not Workspace:FindFirstChild("Hitboxes") then
    print(GlobalText.GameNotLoaded2)
    Workspace:WaitForChild("Hitboxes")
end
if CanHookFunc then
    OldHitboxesDestroy = Hook(Workspace.Hitboxes.Destroy, function() end)
end

if getgenv().MFeeeLoaded then
    return MiniNotify(GlobalText.ScriptLoaded)
elseif getgenv().MFeeeLoading then
    return MiniNotify(GlobalText.ScriptLoading)
end
...
--// Beautiful Arts
local Arts = {-- https://patorjk.com/software/taag
    [[
...
SpawnLoad(function()
    LatestVersion = loadstring(game:HttpGet(PastebinRepo .. "BhnD85nE"))("Weaponry")
end)
...
makefolder("MFeee~ Project")
local TimeOutLimiteFilePath, TimeOutLimite, LoadTimer = "MFeee~ Project/TimeOutLimite.txt"
...
local Remotes = Cloneref(game:GetService("ReplicatedStorage")):WaitForChild("Remotes")
...
local MainWindow = Library:CreateWindow({
    Title = GlobalText.MainWindowTitleWeaponry .. Emoji,
...
local Tabs = {
    ...
    Combat = MainWindow:AddTab(GlobalText.CombatTab, "swords"),
...
AntiAFKToggle = MainAFKGroupbox:AddToggle("AntiAFKToggle", {
    ...
    Callback = function(Enabled)
        ...
        pcall(function()
            for _, Object in pairs(GetConnections(Remotes.AFK.OnClientEvent)) do
...
AntiKickToggle = MainOthersGroupbox:AddToggle("AntiKickToggle", {
    ...
    Default = true,
    Disabled = true,
    DisabledTooltip = GlobalText.ForYourSafety,
    Callback = function()
        Universals.AntiKick(true)
    end
})
Universals.AntiKick(true)
pcall(function()
    for _, Object in pairs(GetConnections(Remotes.AntiExploit.OnClientEvent)) do
        if Object["Disable"] then
            Object["Disable"](Object)
        elseif Object["Disconnect"] then
            Object["Disconnect"](Object)
...
--// Body Movers Fly Tab
BodyMoversFlyTab = FlyTabs:AddTab(GlobalText.FlyGruopbox)
FlyToggle = BodyMoversFlyTab:AddToggle("FlyToggle", {
...
FlySpeedSlider = BodyMoversFlyTab:AddSlider("FlySpeedSlider", {
...
VerticalFlySpeedMultiplierSlider = BodyMoversFlyTab:AddSlider("VerticalFlySpeedMultiplierSlider", {
...
FlyModeDropdown = BodyMoversFlyTab:AddDropdown("FlyModeDropdown", {
...
BodyMoversFlyTab:AddDivider()
FlyGyroToggle = BodyMoversFlyTab:AddToggle("FlyGyroToggle", {
...
SmoothGyroToggle = BodyMoversFlyTab:AddToggle("SmoothGyroToggle", {
...
SmoothGyroValueSlider = BodyMoversFlyTab:AddSlider("SmoothGyroValueSlider", {
...
BodyMoversFlyTab:AddDivider()
FlyControllerDropdown = BodyMoversFlyTab:AddDropdown("FlyControllerDropdown", {
...
UseUpVectorFlyToggle = BodyMoversFlyTab:AddToggle("UseUpVectorFlyToggle", {
...
StopFlyOnDiedToggle = BodyMoversFlyTab:AddToggle("StopFlyOnDiedToggle", {
...
local function ESPConditions(Player)
    return Player and not (AllowESP and (ESPMe or Player ~= Speaker) and (not ESPTeamCheck or #Teams:GetTeams() < 2 or Player.TeamColor.Color ~= Speaker.TeamColor.Color))
end
...
--// Weaponry Combat Module
local DontChangeMyHitBox, CombatConnections, Combat = true, {}, {
    HitBoxMelee = true,
    HitBoxBody = true,
    HitBoxHead = true,
    HitBoxLeg = true,
    HitBoxMeleeMultiplier = 1,
    HitBoxBodyMultiplier = 1,
    HitBoxHeadMultiplier = 1,
    HitBoxLegMultiplier = 1,
}

local function Multi(Object)
    pcall(function()
        if DontChangeMyHitBox and tostring(Object.Parent.Name) == tostring(Speaker.UserId) then return end
        if Object.Name == "HitboxMelee" and Combat.HitBoxMelee then
            Object.Size = Vector3.new(6.5, 6, 3) * Combat.HitBoxMeleeMultiplier
        elseif Object.Name == "HitboxBody" and Combat.HitBoxBody then
            Object.Size = Vector3.new(4.1, 2, 1.1) * Combat.HitBoxBodyMultiplier
        elseif Object.Name == "HitboxHead" and Combat.HitBoxHead then
            Object.Size = Vector3.new(1.2, 1.1, 1.1) * Combat.HitBoxHeadMultiplier
        elseif Object.Name == "HitboxLeg" and Combat.HitBoxLeg then
            Object.Size = Vector3.new(2.1, 2, 1.1) * Combat.HitBoxLegMultiplier
        end
    end)
end

local function HitBoxMultiplier(Enabled)
    HitBoxEnabled = Enabled
    if Enabled then
        for _, Object in pairs(Workspace.Hitboxes:GetDescendants()) do
            Multi(Object)
        end
        CombatConnections.Hitboxes = (CombatConnections.Hitboxes and CombatConnections.Hitboxes:Disconnect()) or Workspace.Hitboxes.DescendantAdded:Connect(function(Object)
            if not HitBoxEnabled then
                return CombatConnections.Hitboxes:Disconnect()
            end
            Multi(Object)
        end)
    else
        CombatConnections.Hitboxes = (CombatConnections.Hitboxes and CombatConnections.Hitboxes:Disconnect())
        for _, Object in pairs(Workspace.Hitboxes:GetDescendants()) do
            if Object.Name == "HitboxMelee" then
                Object.Size = Vector3.new(6.5, 6, 3)
            elseif Object.Name == "HitboxBody" then
                Object.Size = Vector3.new(4.1, 2, 1.1)
            elseif Object.Name == "HitboxHead" then
                Object.Size = Vector3.new(1.2, 1.1, 1.1)
            elseif Object.Name == "HitboxLeg" then
                Object.Size = Vector3.new(2.1, 2, 1.1)
            end
        end
    end
end
...
do
    local PlayerIds = {}
    for _, Player in pairs(Players:GetPlayers()) do
        table.insert(PlayerIds, tonumber(Player.UserId))
    end
    for _, Folder in pairs(Workspace.Hitboxes:GetChildren()) do
        if Folder:IsA("Folder") and not table.find(PlayerIds, tonumber(Folder.Name)) then
            Folder:Destroy()
        end
    end
end

CombatConnections.PR = (CombatConnections.PR and CombatConnections.PR:Disconnect()) or Players.PlayerRemoving:Connect(function(Player)
    local Folder = Workspace.Hitboxes:FindFirstChild(tostring(Player.UserId))
    if Folder then
        Folder:Destroy()
    end
end)

--// Combat Exploit Groupbox
CombatHitBoxGroupbox = Tabs.Combat:AddLeftGroupbox(GlobalText.CombatHitBoxGroupbox)
DontChangeMyHitBoxToggle = CombatHitBoxGroupbox:AddToggle("DontChangeMyHitBoxToggle", {
    Text = GlobalText.DontChangeMyHitBoxToggle,
    Default = true,
    Callback = function(Enabled)
        DontChangeMyHitBox = Enabled
        if HitBoxMultiplierToggle.Value then
            task.spawn(function()
                HitBoxMultiplierToggle:SetValue(false)
                Heartbeat:Wait()
                HitBoxMultiplierToggle:SetValue(true)
            end)
        end
        if HitBoxTransparencyToggle.Value then
            HitBoxTransparencyToggle:SetValue(false)
            Heartbeat:Wait()
            HitBoxTransparencyToggle:SetValue(true)
        end
    end
})
HitBoxMultiplierToggle = CombatHitBoxGroupbox:AddToggle("HitBoxMultiplierToggle", {
    Text = GlobalText.HitBoxMultiplierToggle,
    Default = false,
    Callback = function(Enabled)
        HitBoxMultiplier(Enabled)
    end
})
HitBoxPartsDropdown = CombatHitBoxGroupbox:AddDropdown("HitBoxPartsDropdown", {
    Text = GlobalText.HitBoxPartsDropdown,
    Values = {
        GlobalText.HitBoxMelee,
        GlobalText.HitBoxBody,
        GlobalText.HitBoxHead,
        GlobalText.HitBoxLeg
    },
    Default = {
        GlobalText.HitBoxMelee,
        GlobalText.HitBoxBody,
    ...
    Callback = function(Parts)
        Combat.HitBoxMelee = table.find(Parts, GlobalText.HitBoxMelee)
        Combat.HitBoxBody = table.find(Parts, GlobalText.HitBoxBody)
        Combat.HitBoxHead = table.find(Parts, GlobalText.HitBoxHead)
        Combat.HitBoxLeg = table.find(Parts, GlobalText.HitBoxLeg)
    end
})
CombatHitBoxGroupbox:AddDivider()
local function HitBoxSlide(Text, Which)
    local Number = tonumber(Text)
    Combat[Which] = Number or Combat[Which]
    if not HitBoxMultiplierToggle.Value then return end
    for _, Object in pairs(Workspace.Hitboxes:GetDescendants()) do
        Multi(Object)
    end
end
HitBoxMeleeMultiplierSlider = CombatHitBoxGroupbox:AddSlider("HitBoxMeleeMultiplierSlider", {
    Text = GlobalText.HitBoxMeleeMultiplierInput,
    Default = 1,
    Max = 40,
    Min = 1,
    Rounding = 1,
    ...
    HideMax = true,
    Callback = function(Number)
        HitBoxSlide(Number, "HitBoxMeleeMultiplier")
    end
})
HitBoxHeadMultiplierSlider = CombatHitBoxGroupbox:AddSlider("HitBoxHeadMultiplierSlider", {
    Text = GlobalText.HitBoxHeadMultiplierInput,
    Default = 1,
    Max = 40,
    Min = 1,
    Rounding = 1,
    ...
    HideMax = true,
    Callback = function(Number)
        HitBoxSlide(Number, "HitBoxHeadMultiplier")
    end
})
HitBoxBodyMultiplierSlider = CombatHitBoxGroupbox:AddSlider("HitBoxBodyMultiplierSlider", {
    Text = GlobalText.HitBoxBodyMultiplierInput,
    Default = 1,
    Max = 40,
    Min = 1,
    Rounding = 1,
    ...
    HideMax = true,
    Callback = function(Number)
        HitBoxSlide(Number, "HitBoxBodyMultiplier")
    end
})
HitBoxLegMultiplierSlider = CombatHitBoxGroupbox:AddSlider("HitBoxLegMultiplierSlider", {
    Text = GlobalText.HitBoxLegMultiplierInput,
    Default = 1,
    Max = 40,
    Min = 1,
    Rounding = 1,
    ...
    HideMax = true,
    Callback = function(Number)
        HitBoxSlide(Number, "HitBoxLegMultiplier")
    end
})
CombatHitBoxGroupbox:AddDivider()
local function HitBoxTrans(Number, Which)
    for _, Object in pairs(Workspace.Hitboxes:GetDescendants()) do
        if DontChangeMyHitBox and tostring(Object.Parent.Name) == tostring(Speaker.UserId) then return end
        if Object.Name == Which then
            Object.Transparency = Number / 100
        end
    end
end
HitBoxTransparencyToggle = CombatHitBoxGroupbox:AddToggle("HitBoxTransparencyToggle", {
    Text = GlobalText.HitBoxTransparencyToggle,
    Default = false,
    Callback = function(Enabled)
        if Enabled then
            local function Handle(Object)
                ...
                if Object.Name == "HitboxMelee" then
                    Object.Transparency = Options.HitBoxMeleeTransparencySlider.Value / 100
                elseif Object.Name == "HitboxHead" then
                    Object.Transparency = Options.HitBoxHeadTransparencySlider.Value / 100
                elseif Object.Name == "HitboxBody" then
                    Object.Transparency = Options.HitBoxBodyTransparencySlider.Value / 100
                elseif Object.Name == "HitboxLeg" then
                    Object.Transparency = Options.HitBoxLegTransparencySlider.Value / 100
                end
            end
            for _, Object in pairs(Workspace.Hitboxes:GetDescendants()) do
                Handle(Object)
            end
            CombatConnections.HT = (CombatConnections.HT and CombatConnections.HT:Disconnect()) or Workspace.Hitboxes.DescendantAdded:Connect(function(Object)
                Handle(Object)
            end)
        else
            CombatConnections.HT = (CombatConnections.HT and CombatConnections.HT:Disconnect())
            for _, Object in pairs(Workspace.Hitboxes:GetDescendants()) do
                if Object.Name == "HitboxMelee" then
...
HitBoxMeleeTransparencySlider = CombatHitBoxGroupbox:AddSlider("HitBoxMeleeTransparencySlider", {
    Text = GlobalText.HitBoxMeleeTransparencySlider,
    Default = 65,
    Min = 0,
    Max = 100,
    Rounding = 0,
    ...
    HideMax = false,
    Callback = function(Number)
        HitBoxTrans(Number, "HitboxMelee")
    end
})
HitBoxHeadTransparencySlider = CombatHitBoxGroupbox:AddSlider("HitBoxHeadTransparencySlider", {
    Text = GlobalText.HitBoxHeadTransparencySlider,
    Default = 65,
    Min = 0,
    Max = 100,
    Rounding = 0,
    ...
    HideMax = false,
    Callback = function(Number)
        HitBoxTrans(Number, "HitboxHead")
    end
})
HitBoxBodyTransparencySlider = CombatHitBoxGroupbox:AddSlider("HitBoxBodyTransparencySlider", {
    Text = GlobalText.HitBoxBodyTransparencySlider,
    Default = 65,
    Min = 0,
    Max = 100,
    Rounding = 0,
    ...
    HideMax = false,
    Callback = function(Number)
        HitBoxTrans(Number, "HitboxBody")
    end
})
HitBoxLegTransparencySlider = CombatHitBoxGroupbox:AddSlider("HitBoxLegTransparencySlider", {
    Text = GlobalText.HitBoxLegTransparencySlider,
    Default = 65,
    Min = 0,
    Max = 100,
    Rounding = 0,
    ...
    HideMax = false,
    Callback = function(Number)
        HitBoxTrans(Number, "HitboxLeg")
    end
})

--// Combat Other Groupbox
CombatOtherGroupbox = Tabs.Combat:AddRightGroupbox(GlobalText.CombatOtherGroupbox)
APlayerDropdown = CombatOtherGroupbox:AddDropdown("APlayerDropdown", {
...
LockToPlayerToggle = CombatOtherGroupbox:AddToggle("LockToPlayerToggle", {
...
HeadSitPlayerToggle = CombatOtherGroupbox:AddToggle("HeadSitPlayerToggle", {
...
CombatOtherGroupbox:AddDivider()
FlingToggle = CombatOtherGroupbox:AddToggle("FlingToggle", {
    Text = GlobalText.FlingToggle,
    Default = false,
    Callback = function(Enabled)
        GlobalFlinging = Enabled
        UniversalModules.WalkFling(Enabled)
    end
...
UnloadButton = MenuGroup:AddButton({
    ...
    Func = function()
        ...
        Disconnect(MFCons)
        Disconnect(CombatConnections)
        if CanHookFunc and type(OldHitboxesDestroy) == "function" then
            Hook(Workspace.Hitboxes.Destroy, OldHitboxesDestroy)
        end
        getgenv().MFeeeLoaded = false
        getgenv().MFeeeLoading = false
...
SaveManager:SetFolder("MFeee~ Project/MFeee~ Weaponry")
